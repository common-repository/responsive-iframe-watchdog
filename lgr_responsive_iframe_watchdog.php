...
Author URI: https://www.lgr-wp.com/wp/?page_id=4638
...
function lgrriw_settings_init(  ) { 
		...
		global $pluginName;
		$pluginName = lgrriw_GetMyPluginFolderNameFromFilePath(1);
	 	wp_enqueue_style( 'lgrriw_responsive_iframe_watchdog', plugins_url( $pluginName . 'admin/css/lgr_responsive_iframe_watchdog.css' ) );
		//wp_enqueue_script( 'lgrriw_responsive_iframe_watchdog', plugins_url( $pluginName . 'admin/js/lgr_responsive_iframe_watchdog.js' ) );
...
function lgrriw_GetMyPluginFolderNameFromFilePath( $moveUpFolderCount ){
	$arrayMyPluginName = explode( '/', plugin_dir_path( __FILE__ ) );
	$pluginNameIndex = sizeof($arrayMyPluginName) - $moveUpFolderCount -1;
	return $arrayMyPluginName[$pluginNameIndex] . '/'; 
}
